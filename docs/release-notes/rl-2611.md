...
- {file}`hm-session-vars.sh` is now safe to re-source, so new Bash, Zsh, and
  ...
  missing entries without moving existing ones. Nested Bash shells preserve
  values owned by Bash login configuration; pre-manifest sessions and Bash
  versions before 4.2 do so conservatively until the next login. Interactive
