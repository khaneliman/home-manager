...
def extract_maintainers(changed_files: list[str], pr_author: str) -> list[str]:
    ...
    if not changed_files:
        print("No module files changed. No maintainers to tag.", file=sys.stderr)
        return []

    print("Finding maintainers for changed files...", file=sys.stderr)

    file = Path(__file__).parent.parent / "nix" / "extract-maintainers.nix"
    result = run_nix_eval(
        str(file), "--argstr", "changedFilesJson", json.dumps(changed_files)
    )
