  ...
  generateSourceLines = sources:
    let
      generateCommand = source:
        ...
        else if (isString source || isPath source)
        then "source " + source
  ...
  config = mkIf cfg.enable (mkMerge ([
    { home.packages = [ cfg.package ]; }
    {
      xdg.configFile."sketchybar/sketchybarrc".source = pkgs.writeShellScript "sketchybarrc" ("\n"
        ...
        + optionalString (cfg.config.bar != { }) "sketchybar --bar \\\n${toSketchybarConfig cfg.config.bar}\n\n"
        + optionalString (cfg.config.defaults != { }) "sketchybar --default \\\n${toSketchybarConfig cfg.config.defaults}\n\n"
