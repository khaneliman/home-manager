...
in
{
  ...
  config = lib.mkIf cfg.enable {
        ...
        luaPaths = lib.filter (x: x != "") [
          ...
          (lib.optionalString (cfg.configType == "lua" && cfg.config != null) (
            let
              ...
              fileContent = lib.hm.types.fileContentToHomeFile cfg.config;
              isRecursiveDir = fileContent ? source && fileContent ? recursive && fileContent.recursive;
            in
    ...
    xdg.configFile = lib.mkIf (cfg.config != null) (
      let
        # Enhanced config with automatic script generation
        defaultTemplate = {
          header =
            if cfg.configType == "lua" then "-- Generated by home-manager" else "# Generated by home-manager";
        };
        ...
        enhancedConfig =
          if lib.isString cfg.config then
            {
              text = cfg.config;
              scriptType = cfg.configType;
              template = defaultTemplate;
            }
          else if cfg.config ? text && cfg.config.text != null then
            cfg.config
            // {
              scriptType = cfg.config.scriptType or cfg.configType;
              template = cfg.config.template or defaultTemplate;
            }
          else
            cfg.config;

        fileContent = lib.hm.types.fileContentToHomeFile enhancedConfig;

        # If it's a recursive directory, place it at the sketchybar root
        # Otherwise create the sketchybarrc file
        isRecursiveDir = fileContent ? source && fileContent ? recursive && fileContent.recursive;
      in
      if isRecursiveDir then
        {
          "sketchybar" = fileContent;
        }
      else
        {
          "sketchybar/sketchybarrc" = fileContent;
        }
    );
  };
}
