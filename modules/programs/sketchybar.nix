...
in
{
  ...
  options.programs.sketchybar = {
    ...
    config = mkOption {
      type = types.nullOr lib.hm.types.fileContent;
      default = null;
  ...
  config = lib.mkIf cfg.enable {
    ...
    programs.sketchybar.finalPackage =
      let
        ...
        luaPaths = lib.filter (x: x != "") [
          ...
          (lib.optionalString (cfg.configType == "lua" && cfg.config != null) (
            let
              configDir = "${config.xdg.configHome}/sketchybar";

              # `cfg.config` is already the final, processed fileSpec object.
              # We can just read its `source` and `recursive` properties directly.
              isRecursiveDir = cfg.config.source != null && cfg.config.recursive;
            in
            if isRecursiveDir then
    ...
    xdg.configFile = lib.mkIf (cfg.config != null) (
      let
        # Simple approach: if we have text content, add the script headers
        processedText =
          if cfg.config.text != null then
            let
              shebangMap = {
              ...
              shebang = shebangMap.${cfg.configType};
              comment = "${commentMap.${cfg.configType}} Generated by home-manager";
            in
        ...
        homeFileAttrs = {
          executable = if cfg.config.text != null then true else (cfg.config.executable or false);
          recursive = cfg.config.recursive or false;
        }
        // lib.optionalAttrs (processedText != null) { text = processedText; }
        // lib.optionalAttrs (cfg.config.source != null) { source = cfg.config.source; };

        isRecursiveDir = (cfg.config.source != null) && (cfg.config.recursive or false);
      in
      if isRecursiveDir then
        {
          "sketchybar" = homeFileAttrs;
        }
      else
        {
          "sketchybar/sketchybarrc" = homeFileAttrs;
        }
    );
  };
}
