...
let
  ...
  toSketchybarConfig = opts:
    let
      ...
      lastLine = "${lastPair}";
    in lib.concatStrings (linesWithQuotes ++ [ lastLine ]);

  exportVariables = variables:
    lib.concatStringsSep "\n" (builtins.attrValues
      (builtins.mapAttrs (name: value: "export ${name}=${value}") variables));

  generateSourceLines = sources:
    let
          ...
          let
            ...
            keyValuePairs = toString
              (lib.mapAttrs (key: value: "${key}=${value}")
                (removeAttrs source [ "name" "placement" ]));
          in "sketchybar --add item ${name} ${placement} --set ${name} ${keyValuePairs}"
...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    xdg.configFile."sketchybar/sketchybarrc".source =
      pkgs.writeShellScript "sketchybarrc" ("\n"
