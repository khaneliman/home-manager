...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    xdg.configFile = lib.mkIf (cfg.config != null) (
      let
        # The `cfg.config` value is ALREADY the final, fully-processed result from our "smart" type.
        # It has the correct `text`, `executable` bit, etc., all set.
        finalFileSpec = cfg.config;

        # We just need to know if it's a recursive directory to set the target path correctly.
        isRecursiveDir = finalFileSpec.source != null && finalFileSpec.recursive;
      in
      if isRecursiveDir then
        {
          # If it's a directory, the target is "sketchybar"
          "sketchybar" = lib.hm.types.fileSpecToHomeFileAttrs finalFileSpec;
        }
      else
        {
          # Otherwise, it's a single file named "sketchybarrc"
          "sketchybar/sketchybarrc" = lib.hm.types.fileSpecToHomeFileAttrs finalFileSpec;
        }
    );
  };
}
