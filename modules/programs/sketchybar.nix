...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    xdg.configFile = lib.mkIf (cfg.config != null) (
      let
        # Add scriptType to config so fileContent type can process it
        configWithScriptType = cfg.config // { scriptType = cfg.configType; };
        isRecursiveDir = configWithScriptType.source != null && configWithScriptType.recursive;
      in
      if isRecursiveDir then
        {
          "sketchybar" = lib.hm.types.fileSpecToHomeFileAttrs configWithScriptType;
        }
      else
        {
          "sketchybar/sketchybarrc" = lib.hm.types.fileSpecToHomeFileAttrs configWithScriptType;
        }
    );
  };
}
