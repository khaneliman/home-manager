...
in {
  ...
  options.services.plan9port = {
    fontsrv.enable =
      lib.mkEnableOption "the Plan 9 file system access to host fonts";
    plumber.enable =
      lib.mkEnableOption "the Plan 9 file system for interprocess messaging";
  };

  config = lib.mkIf (cfg.fontsrv.enable || cfg.plumber.enable) {
    assertions = [
      (lib.hm.assertions.assertPlatform "services.plan9port" pkgs
        lib.platforms.linux)
    ];
