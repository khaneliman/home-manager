...
let
  inherit (lib) concatLists mkIf mkOption types;
  ...
  convertPolybarKeyVal = key: val:
    ...
    if lib.isList val then
      concatLists
      (lib.imap0 (i: convertPolybarKeyVal "${key}-${toString i}") val)
      # Convert {
      #   foo.text = "a";
      #   foo.font = 1;
      # } to {
      ...
      concatLists (lib.mapAttrsToList
  ...
  convertPolybarSection = _: attrs:
    lib.listToAttrs
    (concatLists (lib.mapAttrsToList convertPolybarKeyVal attrs));
  ...
  toPolybarIni = lib.generators.toINI {
    mkKeyValue = key: value:
      let
        quoted = v:
          if lib.hasPrefix " " v || lib.hasSuffix " " v then ''"${v}"'' else v;
