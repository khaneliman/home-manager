...
let
  inherit (lib) generators isAttrs literalExpression mkOption types;
  ...
  mkRecollConfKeyValue = generators.mkKeyValueDefault {
    mkValueString = let mkQuoted = v: ''"${lib.escape [ ''"'' ] v}"'';
    in v:
    if v == true then
      "1"
  ...
  toRecollConf = { listsAsDuplicateKeys ? false }:
    ...
    let
      ...
      # come last.
      _config =
        lib.mapAttrsToList convert (lib.filterAttrs (k: v: !isAttrs v) attr);
      _config' =
        lib.mapAttrsToList convert (lib.filterAttrs (k: v: isAttrs v) attr);
