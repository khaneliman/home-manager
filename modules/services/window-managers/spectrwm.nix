{ config, lib, pkgs, ... }:
let
  inherit (lib) literalExpression mapAttrsToList mkOption types;

  cfg = config.xsession.windowManager.spectrwm;

  renderedValue = val:
    if lib.isBool val then if val then "1" else "0" else toString val;
...
in {
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      (lib.hm.assertions.assertPlatform "xsession.windowManager.spectrwm" pkgs
        lib.platforms.linux)
    ];
